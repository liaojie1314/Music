...
import com.example.music.manager.interfaces.CloudMusicManagerInterface
...
class CloudMusicManager: CloudMusicManagerInterface {
    ...
    override fun getUserDetail(
        uid: String,
        success: (result: com.example.music.data.UserDetailData) -> Unit,
        failure: (error: String) -> Unit
    ) {
        MagicHttp.OkHttpManager().newGet("${API_MUSIC_API}/user/detail?uid=$uid", {
            try {
                val userDetailData = Gson().fromJson(it, com.example.music.data.UserDetailData::class.java)
